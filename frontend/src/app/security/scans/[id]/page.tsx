...
function ScannedFilesPanel({ files, partial }: { files: string[]; partial: boolean }) {
  const [showAll, setShowAll] = useState(false);
  if (files.length === 0) return null;
  const visible = showAll ? files : files.slice(0, 200);
  return (
    <div className="border-t border-border-soft">
      <div className="px-4 py-3 flex items-center justify-between bg-surface-base">
        <h4 className="text-[13px] font-semibold text-text-primary">Scanned Files</h4>
        <span className="text-[12px] text-text-secondary font-mono bg-surface-elevated px-2 py-0.5 rounded border border-border-soft">{files.length}</span>
      </div>
      {partial && (
        <div className="px-4 py-2 text-[12px] text-status-high border-b border-border-soft bg-surface-elevated/30">
          Scan did not complete — listing the files scanned before the error.
        </div>
      )}
      <ul className="px-4 py-2 max-h-72 overflow-y-auto divide-y divide-border-soft">
        {visible.map((f) => (
          <li key={f} className="py-1.5 text-[11px] font-mono text-text-secondary truncate">{f}</li>
        ))}
      </ul>
      {files.length > visible.length && (
        <div className="px-4 pb-3">
          <button onClick={() => setShowAll(!showAll)} className="text-[12px] text-accent hover:underline">
          ...
          <Card className="p-0 overflow-hidden h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto bg-page-bg">
              {scan.findings.length === 0 ? (
                (scan.scanned_files?.length ?? 0) > 0 ? (
                  <ScannedFilesPanel files={scan.scanned_files as string[]} partial={scan.status === "failed"} />
                ) : (
                ...
                <div className="flex flex-col">
                  ...
                  <ScannedFilesPanel files={scan.scanned_files ?? []} partial={scan.status === "failed"} />
                </div>
              )}
            </div>
          </Card>
